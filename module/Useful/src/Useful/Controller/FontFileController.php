...
class FontFileController {
	...
	public function getFontInfo() {
		...
		for($i = 0; $i < $number_of_tables; $i ++) {
			...
			if ($tag == 'name') {
				$this->ntOffset = hexdec ( $this->dec2ord ( $this->text [12 + $i * 16 + 8] ) . $this->dec2ord ( $this->text [12 + $i * 16 + 8 + 1] ) . $this->dec2ord ( $this->text [12 + $i * 16 + 8 + 2] ) . $this->dec2ord ( $this->text [12 + $i * 16 + 8 + 3] ) );
