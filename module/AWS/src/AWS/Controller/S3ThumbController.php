...
class S3ThumbController
{
    ...
    public function createCanvas(array $params)
    {
        ...
        $color = isset($this->_fileInfo['color']) ? $this->_fileInfo['color'] : FALSE;
        ...
        if (! is_readable($this->_tmpDir)) {
            throw new \Exception('A pasta temp não está habilitada para leitura' . $this->_tmpDir);
        } else 
            if (! is_writable($this->_tmpDir)) {
                throw new \Exception('A pasta temp não está habilitada para escrita' . $this->_tmpDir);
            }
     ...
     *            Permissoes disponíveis private, public-read, public-read-write
     */
    public function putThumb(array $fileInfo, $nameDestinationThumb, $permission)
    {
        ...
        $s3 = new S3Controller($this->_config["AwsS3"]["key"], $this->_config["AwsS3"]["secret"]);
        // $s3->putBucket ( $this->_bucket, S3Controller::ACL_PRIVATE );
        
        if ($s3->putObjectFile($resource, $this->_bucket, $nameDestinationThumb, S3Controller::ACL_PUBLIC_READ)) {
            unlink($resource);
            return true;
        } else {
            unlink($resource);
